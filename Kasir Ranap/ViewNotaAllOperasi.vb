...
Public Class ViewNotaAllOperasi

    Public Ambil_Data As String
    Public Form_Ambil_Data As String

    Private Sub ViewNotaAllOperasi_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If Ambil_Data = True Then
            Select Case Form_Ambil_Data
                Case "OperasiAll"
                    ...
                    da = New MySqlDataAdapter("SELECT noTindakanOP,tindakan,jmlTindakan,
	                                                  tarif,subTotal 
                                                 FROM vw_tindakanpasienopdetail
                                                WHERE noDaftar = '" & Home.txtNoReg.Text & "'", conn)
                    ds = New DataSet
                    da.Fill(dt)
                    ReportViewer1.LocalReport.DataSources.Clear()
                    Dim rpt As New ReportDataSource("CetakNotaOperasi", dt)
                ...
                Case "OperasiPernota"
                    ...
                    da = New MySqlDataAdapter("SELECT noTindakanOP,tindakan,jmlTindakan,
	                                                  tarif,subTotal 
                                                 FROM vw_tindakanpasienopdetail
                                                WHERE noTindakanOP = '" & Home.txtNoTinOp.Text & "'", conn)
                    ds = New DataSet
                    da.Fill(dt)
                    ReportViewer1.LocalReport.DataSources.Clear()
                    Dim rpt As New ReportDataSource("CetakNotaOperasi", dt)
