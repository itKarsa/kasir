...
Public Class Laboratorium
    ...
    Sub tampilData()
        ...
        da = New MySqlDataAdapter("SELECT noRegistrasiPenunjangRanap,tglMasukPenunjangRanap,
                                 ...
                                 ORDER BY tglMasukPenunjangRanap ASC", conn)
        ds = New DataSet
        da.Fill(ds, "vw_datalabpasienranap")
        DataGridView1.DataSource = ds.Tables("vw_datalabpasienranap")
        Call aturDGV()
    End Sub

    Sub aturDGV()
        'DataGridView1.Columns(0).Width = 150
        DataGridView1.Columns(0).HeaderText = "NO.DAFTAR LAB"
    ...
    Sub tampilDataDetail()
        ...
        da = New MySqlDataAdapter("SELECT kdTarif, tindakan, jumlahTindakan, tarif, totalTarif 
                                     FROM vw_datalabpasienranap WHERE noTindakanPenunjangRanap = '" & txtNoTindakan.Text & "'", conn)
        ds = New DataSet
        da.Fill(ds, "vw_datalabpasienranap")
    ...
    Sub updatePembayaran()
        ...
        str = "UPDATE t_tindakanpenunjangranap 
                  SET statusPembayaran = '" & txtStatus.Text & "',
                      tglPembayaran = '" & dt & "'
                WHERE noTindakanPenunjangRanap = '" & txtNoTindakan.Text & "'"

        Call koneksiServer()
        Try
            cmd = New MySqlCommand(str, conn)
            cmd.ExecuteNonQuery()
            MsgBox("Pembayaran Tindakan Lab berhasil dilakukan", MessageBoxIcon.Information)
    ...
    Private Sub Laboratorium_Closing(sender As Object, e As CancelEventArgs) Handles Me.Closing
    ...
    Private Sub Laboratorium_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        Form1.Show()
    End Sub
End Class