...
Public Class ViewNotaAllObat

    Public Ambil_Data As String
    Public Form_Ambil_Data As String

    Private Sub ViewNotaAllObat_Load(sender As Object, e As EventArgs) Handles MyBase.Load
            ...
            Select Case Form_Ambil_Data
                Case "ObatAll"
                    ...
                    da = New MySqlDataAdapter("SELECT
	                                                noPenjualanObatRajal,
	                                                namaObat,
	                                                JumlahItem,
	                                                harga,
	                                                subTotalPenjualan 
                                                FROM
	                                                vw_penjualanobatdetail
                                               WHERE 
                                                    noDaftar = '" & Home.txtNoReg.Text & "'", conn)
                    ds = New DataSet
                    da.Fill(dt)
                    ReportViewer1.LocalReport.DataSources.Clear()
                    Dim rpt As New ReportDataSource("CetakNotaObat", dt)
                ...
                Case "ObatPernota"
                    ...
                    da = New MySqlDataAdapter("SELECT
                                                ...
                                                FROM
	                                                vw_penjualanobatdetail
                                                WHERE
	                                                noPenjualanObatRajal = '" & Home.txtNoJualObat.Text & "'", conn)
                    ds = New DataSet
                    da.Fill(dt)
                    ReportViewer1.LocalReport.DataSources.Clear()
                    Dim rpt As New ReportDataSource("CetakNotaObat", dt)
